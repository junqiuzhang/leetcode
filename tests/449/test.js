...
it('449: serialize', () => {
  expect(serialize).call(array2tree([])).toBe('[]');
  expect(serialize)
    .call(array2tree([2, 1, 3]))
    .toBe('[2,1,3]');
  expect(serialize)
    .call(array2tree([2, 1, 3, null, null, 4]))
    .toBe('[2,1,3,null,null,4]');
  expect(serialize)
    .call(array2tree([1, 2, 3, 4, 5, 6, 7, 8, 9]))
    .toBe('[1,2,3,4,5,6,7,8,9]');
});
it('449: deserialize', () => {
  expect(deserialize).call('[]').toBe(array2tree([]));
  expect(deserialize)
    .call('[2,1,3]')
    .toBe(array2tree([2, 1, 3]));
  expect(deserialize)
    .call('[2,1,3,null,null,4]')
    .toBe(array2tree([2, 1, 3, null, null, 4]));
  expect(deserialize)
    .call('[1,2,3,4,5,6,7,8,9]')
    .toBe(array2tree([1, 2, 3, 4, 5, 6, 7, 8, 9]));
});
